...
public class Bin2granite {
  ...
  // assumes there is a log file. If using through ascii2bin, this is not a concern.
  public Bin2granite(Ascii2bin in)
  {
    ...
    for (int i = 0; i < in.headers.size(); i++)
      xfdls[i] = new String(in.filename + "_" + in.headers.get(i).signature + ".xfdl");
  ...
  public boolean makeXFiles() {
    ...
    for (int i = 0, count = 0; i < in.headers.size(); i++)
    {
      ...
      // using some math trickery to calculate array boundaries
      ...
      else { uppers[1] = uppers[0]; uppers[0] = smallPrime; }
      if (smallPrime == 1) { dim = 1; uppers[0] = n2; }

      bounds.add(new ArrayList<Integer>());
      for (int j = 0; j < dim; j++) // save for granite verification
        bounds.get(count).add(uppers[j]);

      try
      {
        try { new File(xfdls[i]).delete(); } catch (Exception e2) {}

        w = new PrintWriter(new BufferedWriter(new FileWriter(xfdls[i])));

        String rawName = filename;
        if (in.split || in.integrate > 0) rawName = "";
        ...
        for (int j = 0; j < dim; j++)
          w.write(xfdl_granite.replaceAll("UPPER_BOUND", "" + (uppers[j]-1) ));

        String temp = xfdl_field.replaceAll("FIELD_NAME", in.headers.get(i).signature);
        temp = temp.replaceAll("FIELD_TYPE", ((in.floats) ? "float" : "int"));
      ...
      {
        try { w.close(); } catch (Exception e2) {}
    ...
    try
    {
      try { new File(filename).delete(); } catch (Exception e2) {}
      w = new PrintWriter(new BufferedWriter(new FileWriter(filename)));
      w.write(xddl_open);

      for (int i = 0; i < in.headers.size(); i++)
        if (in.skip(i)) continue;
        else w.write(xddl_xfdl.replaceAll("DS_NAME", in.headers.get(i).signature).replaceAll("XFDL_NAME", xfdls[i]));
      ...
      {
        if (in.skip(i)) continue;
        w.write(xddl_component_open.replaceAll("DS_NAME", in.headers.get(i).signature));
        w.write(xddl_field.replaceAll("FIELD_NAME", in.headers.get(i).signature).replaceAll("POSITION", ""+count));
    ...
    {
      try { w.close(); } catch (Exception e2) {}
      Log.log("failed producing xddl file"); return false;
    }
  ...
  public boolean verify()
  {
    try
    {
      ...
      for (int i = 0; i < in.attribsWritten; i++)
      {
        ...
        {
          if (in.skip(j)) continue;

          ds[i] = DataSource.create( xfdls[j], xfdls[j] );
          ds[i].activate();
          r = new RandomAccessFile(in.filename + "_" + in.headers.get(j).signature + ".raw", "r");

          long start = (in.headers.get(j).getField("n1")).longValue(); // start point
          index = 0; rawLen = r.length(); graniteLen = ds[i].volume()*4;

          if ( rawLen != graniteLen ) {
            Log.log("file size mismatch: raw[ " + rawLen + " ] != granite[ " + graniteLen + " ]");
          ...
          while (++index < start)
            if (in.floats) r.readFloat(); else r.readInt();


          // data is there, but something is wrong with endianness, still figuring it out
          Number temp1 = (in.floats) ? new Float(0) : new Integer(0);
          Number [] temp2 = (in.floats) ? new Float[ds[i].getFloats().length] : new Integer[ds[i].getInts().length];
          ...
          for (int l = 0; l < ds[i].getNumAttributes(); l++)
          {
            ...
            {
              if (in.floats) temp1 = r.readFloat();
              else temp1 = r.readInt();

              if ( (in.floats && temp1.floatValue() != temp2[count].floatValue()) ||
                  (!in.floats && temp1.intValue() != temp2[count].intValue())) {
                r.close();
                out("failure at index " + count + " " + temp1 + " != " + temp2[count]);
                return false;
              }
            }
          }
    ...
    catch(Exception e) { Log.log("bad, granite. bad."); return false; }
...
try
{
  ...
  if (file.contains(" ")) {
    if (file.charAt(0) != '\"')
      if (file.charAt( file.length()-1 ) != '\"') file = "\"" + file + "\"";
      else file = "\"" + file;
    else if (file.charAt( file.length()-1 ) != '\"') file += "\"";
  }
}
catch (Exception e) { Log.log(usage); System.exit(1); }*/
