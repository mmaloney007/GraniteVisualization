/**
 * Stephen Dunn Ascii2bin.java
 * 4/5/2013 Description: run with -? for man pages
 */
...
// ascii2bin main program, needs bin2granite.java and Granite to properly
// function
// if you're tracking i/o bugs, remember that input and output use different
// stream types
public class Ascii2bin
{
  // instance vars ------------>
	...
	Bin2granite util;
  boolean exit = false; // will die if true (for threading)
  ...
  final int dimensions; // dimensions if user specified
	final boolean domath; // do the math to generate the floats correctly
	final String[] keep; // keep by attrib. name (keep specified names)
	int attribsWritten = 0;
	
	// the date argument is just me being sneaky so I can timestamp my self on
	// every run
	static final String version = "0.1 (last built 04/16/2013)"; // function
																	// subs this
																	// out below
	static final String usage = "Error: unrecognized arguments (-? for usage)";
	
	static final String fullUsage = "ascii2bin v"
			+ version
			+ "\n"
      ...
      + "\t-dX Specifies number of dimensions for granite output\n"
			...
			+ "Defaults: split=YES, verify=OFF, integrate=NONE, headers=ON, keep=ALL, domath=ON, skip=NONE, granite=OFF";
	
	public Ascii2bin(String filename) throws FileNotFoundException
	{
		this(filename, 0, 0, 3, false, true, false, false, true, null, null);
	}

  public Ascii2bin(String filename, int dimensions, boolean floats) throws FileNotFoundException
  {
    this(filename, 0, 0, dimensions, floats, true, false, false, true, null, null);
  }
	
	/**
	 * Ascii2bin(filename, bool split) split: split multiple attributes into
	 * multiple outputs Reads an input ascii file and prepares a data structure
	 * for binary output
	 * 
	 * @throws FileNotFoundException
	 * */
	public Ascii2bin(String filename, int integrate, int granite, int dimensions,
			boolean floats, boolean split, boolean verify, boolean noheaders, boolean domath,
			boolean[] skip, String[] keep) throws FileNotFoundException
	{
	...
	// (otherwise this function will stamp it's own date format instead of the
	// string!)
	public void stampSource()
	{
		try
		{
			...
			while ((sequence += io.readUTF()) != null)
			{
				String[] split = sequence
						.split("(\\d{2}|M{2})/(\\d{2}|d{2})/(\\d{4}|y{4}])");
				dna = split[0].getBytes("UTF-8").length
		...
		{
		} // if i fail, i fail - shouldn't report to user
	}
	
	// my hopefully legitimate factorization algorithm to determine the largest
	// prime divisor of N = values inputed/attribute (i.e. "-g" is on w/o useDimensions specified)
	// used to set correct boundaries on a DataSource for the sanity of Granite
	// and all of Middle Earth
	public static long largestPrime(long n)
	{
		long max = 1;
		for (long i = 2; i < (n / 2) + 1; i++)
			// don't bother after the half-way mark
			if (n % i == 0)
				max = i;  // congruence check/update
		return max;
	}
	...
	{
		// calculate max number of segments to read because keep/skip may
		// conflict
		int attribsToGo = 0;
		if (keep != null) attribsToGo = keep.length;
		if (skip != null)
		{ // might be overlap here, but eliminated once we pass that index
	...
	public float doMath(int i, int val)
	{
		Float newfloat;
		Float floatval = (float) val;
		Float factor = headers.get(i).getField("factor").floatValue();
		Float hmin = headers.get(i).getField("hmin").floatValue();
		newfloat = hmin + (factor * floatval);
	//	System.err.format("The value of %.2f after multiplication by %.10f and addition to %.10f is: %.10f%n", floatval, factor, hmin, newfloat);
		return newfloat;
	}
	...
	public boolean processFile() throws NullPointerException, IOException
	{
		...
		while (in.hasNext() && !exit)
		{
			...
			{
				if (integrate == 0)
				{ // this is pointless while integrating, not used to break
					// early
				
					int attribsToGo = attribsToWrite(attribute);
					// if # attribs written >= all, done
					if (keep != null && attribsToGo <= attribsWritten)
						break;
				...
				{
					Log.log("failed parsing header for attribute ["
              + attribute + "]");
					close();
					return false;
				}
				...
				if ( skip(attribute) )
				{
					out("Skipping attribute \""
							+ headers.get(attribute).signature + "\" and " + n2
							+ " values...");
					if (floats)
			...
			if (out == null)
			{
				...
				if (split)
					toOpen += "_" + headers.get(attribute).signature; // append
																		// attribute
					
				if (!open(toOpen + ".raw"))
				{
					Log.log("failed creating output file: " + toOpen);
					return false;
				}
				else
					filesGenerated++;
			...
			if (n1 != 1)
			{
				...
				if (floats)
					while (read++ < n1 - 1 && in.hasNextFloat())
						in.nextFloat();
				else
					while (read++ < n1 - 1 && in.hasNextInt())
						in.nextInt();
				
				if (read != n1 - 1)
				{
					Log.log("not enough values for specified N1[ " + n1
							+ " ] at " + headers.get(attribute).signature);
					close();
					attribute++;
					continue;
				}
			...
			while (notDone && !exit)
			{
				try
				{
					// seek by appropriate offset (sizeof(float=int)=4), needed
					// for interleaving data
					if (integrate > 0)
						out.seek((4 * attribsWritten) + (4 * read * integrate));
					read++;
					
					if (floats && domath == false)
					{
					...
					else
					{
						...
						if (domath == true)
						{
							float newVal = doMath(attribute, data);
//							System.err.format("The value of newVal is %f f%n", newVal);
							out.writeFloat(newVal);  //write float
						}
						else
						{
							System.err.println("Write Binary");
							out.writeInt(data); // write binary							
						}
					}
					
					// write padding for later values to have room for
					// interleaving
					if (attribsWritten == 0 && integrate > 0)
					{
						for (int i = 0; i < integrate - 1; i++)
							if (attribsWritten == 0)
								out.writeInt(0); // write 4 bytes
					}
				...
				notDone = ((notDone && n2 == 0) || (notDone && read < n2)) ? true
						: false;
			}
			...
			if (!noheaders)
			{
				// render fields for each attribute - should only see specified
				// attributes
				try
				{
				...
				{
					Log.log("failed rendering attribute counts");
				}
				...
				if (split)
					close(false); // close just the output file
			}
    ...
    if (exit) return false;
		...
		out("Process complete: " + filesGenerated
				+ " raw binary file(s) produced");
		...
		if (granite >= 0)
		{
			...
			if ( util.makeXFiles() )
				out("Granite files sucessfully produced");
			else
			{
				Log.log("failed to generate granite files");
				return false;
			}
		}
		...
		if (verify)
			if (verifyOutput())
				out("Validation succeeded - all data verified");
			else
			{
	...
	class Header
	{
		...
		final ArrayList<Number> vals = new ArrayList<Number>(); // field values
																// in header
		...
		{
			try
			{ // you have better things to do with your time than read this
				// part---
				// just a bunch of string manipulation
				String header = "", headerVals = "";
				
				header = in.nextLine().trim(); // grab signature ("ux", "db",
												// etc.)
				if (header.length() < 2)
				...
				for (int i = 0; i < split.length; i++)
					// save string with clean attributes
					if (split[i].trim().length() > 0)
						names.add(split[i].toLowerCase().trim());
				
				for (int i = 0; i < valueLines; i++)
					// merge all value lines
				...
				for (int i = 0; i < split.length; i++)
					// save string with clean attributes
					if (split[i].trim().length() > 0)
						vals.add(NumberFormat.getInstance().parse(
								split[i].trim()));
			} catch (Exception e)
			{
				throw new NullPointerException("failed parsing header");
			}
		}
	...
	public static void main(String[] args)
	{
		...
		boolean split = true, verify = false, noheaders = true,
        floats = true, domath = true;
		Integer integrate = 0;
		Integer granite = -1; // indicates no granite, 0 = auto
    Integer dimensions= 3; //start with 3
		...
		for (int i = 1; i < args.length; i++)
		{
			...
			if (arg.length() == 2)
			{
				...
				if (arg.charAt(1) == 'm')
				{
					...
					continue;
				} // using floats ON and doing math ON
				if(arg.charAt(1) == 'd')
				{
					noheaders = false;
			...
			else if (arg.charAt(1) == 'i') // integration ON
			{
				...
				try
				{
					integrate = Integer.parseInt(args[i]);
				} catch (NumberFormatException e)
				{
			...
			else if (arg.charAt(1) == 'g')
			{ // make granite files
				noheaders = false;
				args[i] = args[i].substring(2);
				try
				{
					granite = Integer.parseInt(args[i]);
				} 
				catch (NumberFormatException e)
				{
					granite = 0;
			...
			if (arg.substring(1, 5).equals("skip"))
				skipArg = true; // by string or boolean?
			else if (arg.substring(1, 5).equals("keep"))
			{
			} // nothing to do
			else
			{
			...
			ArrayList<String> temp = new ArrayList<String>(); // save everything
																// as a string
																// for now
			...
			if (temp.size() > 0)
			{ // has to be something, i.e. ignore -keep[ ]
				if (skipArg)
				{
					skip = new boolean[temp.size()];
					for (int j = 0; j < skip.length; j++)
						if (temp.get(j).equals("1"))
							skip[j] = true;
						else
							skip[j] = false;
				}
				else
					keep = temp.toArray(new String[temp.size()]);
			}
		}
      ...
      bin = new Ascii2bin(args[0].trim(), integrate.intValue(),
					granite.intValue(), dimensions.intValue(),
          floats, split, verify, noheaders,
          domath, skip, keep);

		} catch (FileNotFoundException e)
		{
			Log.log("failed to read from file \"" + args[0] + "\"");
			System.exit(1);
		} catch (NullPointerException e)
		{
			Log.log("failed to initialize with \"" + args[0] + "\"");
			System.exit(1);
		}
		...
		try
		{
			code = bin.processFile();
		} catch (IOException e)
		{
			Log.log("failed to write output binary file: " + e.getMessage());
			System.exit(1);
		}
		
		if (code)
			out("ascii2bin completed successfully");
		else
			out("ascii2bin completed with errors");
